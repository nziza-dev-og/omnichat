...
export default function ChatInterface({ onContentGenerated }: ChatInterfaceProps) {
  ...
  const handleNewChat = useCallback(() => {
    ...
    if (typeof onContentGenerated === 'function') {
      onContentGenerated(
        "Welcome to OmniAssist! Ask me anything or request code generation.",
        'welcome', // This type will hide the playground
        "Playground"
      );
    }
    if (currentAudio.current) {
  ...
  const handleSendMessage = useCallback(
    async (userInput: string, selectedModelValue: string) => {
      ...
      let assistantResponseMessageContent = ""; // For message display
      let textForTTS = ""; // For text-to-speech
      ...
      try {
        if (selectedModelValue === "together-general") {
          ...
          const langForMarkdown = result.explanation ? codeInput.programmingLanguage : (codeInput.programmingLanguage || 'code');
          assistantResponseMessageContent = `${result.explanation ? `${result.explanation}\n\n` : ''}\`\`\`${langForMarkdown}\n${result.generatedCode}\n\`\`\``;
          textForTTS = result.explanation || `Generated ${codeInput.programmingLanguage} code.`;
          ...
          // Add a temporary message while generating
          ...
          try {
            ...
            // Update the temporary message with the actual image
            setMessages(prev => prev.map(m => m.id === tempImageMsgId ? {...m, content: assistantResponseMessageContent, timestamp: new Date() } : m));
            assistantMessageId = tempImageMsgId; // Use this ID so TTS can be added to this message
          } catch (imgError) {
             ...
             assistantResponseMessageContent = ""; // Clear this so we don't add another message
          }
          
          const newHistoryEntry = `User: ${userInput}\nAssistant (${modelDisplayName}): (Image generated or attempted for prompt: "${userInput}")`;
          setConversationHistory(prev => `${prev}\n${newHistoryEntry}`.trim());
          if (typeof onContentGenerated === 'function') {
            onContentGenerated("", 'image', `Image: ${playgroundTitleBase}`); // Hides playground
          }
        } else {
        ...
        // For images, we updated the temp message. For others, we add a new one.
        ...
        if (existingMessage && selectedModelValue === 'together-image') {
          // Image message (success or failure) was already updated, just add audio if available
           setMessages(prev => prev.map(m => 
            m.id === assistantMessageId 
            ? { ...m, audioDataUri: generatedAudioDataUri } 
            : m
          ));
        } else if (assistantResponseMessageContent) { // For general chat and code
           addMessage({ 
        ...
        // If it was an image generation attempt that failed before updating placeholder
        ...
        if (selectedModelValue === "together-image" && imagePlaceholder) {
          ...
          // Add error message if no assistant message was added/updated yet
